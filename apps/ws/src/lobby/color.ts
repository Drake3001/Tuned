...
function linearToXyz(lin: RGB): XYZ {
  const [r, g, b] = lin;
  return [
...
export function rgbToHsb(rgb: RGB): { hue: number; saturation: number; brightness: number } {
  const r = rgb[0] / 255;
  const g = rgb[1] / 255;
  const b = rgb[2] / 255;
  ...
  if (d === 0) return { hue: 0, saturation: s, brightness: v };
  let h: number;
  switch (max) {
    case r:
      h = (g - b) / d + (g < b ? 6 : 0);
