...
function emitLobbyError(socket, code, message) {
    socket.emit("lobby:error", { code, message });
}
async function assertLobbyMember(code, userId) {
...
export function registerHandlers(io, lobbyManager) {
    io.on("connection", (socket) => {
        ...
        socket.on("player:submit", async (code, guess) => {
            if (typeof code !== "string" || code.length === 0) {
                emitLobbyError(socket, "INVALID_CODE", "Lobby code is required");
                return;
            }
